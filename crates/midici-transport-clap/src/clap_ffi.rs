//! CLAP FFI wrappers — gated behind `clap-ffi` feature.
//!
//! `InputEvents` / `OutputEvents` wrap `clap_input_events_t` /
//! `clap_output_events_t` for SysEx copy/emit on the RT path.
//!
//! `ControlBridge<R>` drives the MIDI-CI engine from a 10 ms host timer.
...
impl InputEvents {
    ...
    pub fn copy_sysex_to<const N: usize, const B: usize>(
        &self,
        _ring: &crate::ring::Ring<N, B>,
        prod: &mut crate::ring::Producer<'_, N, B>,
    ) -> usize {
...
impl OutputEvents {
    ...
    pub fn try_push_sysex(&self, body: &[u8]) -> bool {
        ...
        unsafe {
            ...
            (*p).header.size =
                core::mem::size_of::<clap_event_midi_sysex>() as u32;
            (*p).header.time = 0;
            (*p).header.space_id = 0;
    ...
    pub fn drain_ring<const N: usize, const B: usize>(
        &self,
        cons: &mut crate::ring::Consumer<'_, N, B>,
    ) -> usize {
...
impl<R: rand_core::RngCore> ControlBridge<R> {
    ...
    pub fn tick<const N: usize, const B: usize>(
        &mut self,
        now_ms: u64,
        in_cons: &mut crate::ring::Consumer<'_, N, B>,
        out_prod: &mut crate::ring::Producer<'_, N, B>,
    ) {
